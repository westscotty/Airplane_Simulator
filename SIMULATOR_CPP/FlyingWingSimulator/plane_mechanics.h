...
struct Plane_Mechanics
{
	struct Contoller_Values
	{
		...
		float del_t = 0.0f;    // throttle (0 - 1)   (readVoltage(a, 'A2'))/5;
		float del_a = 0.0f;    // (rad) aileron      (readVoltage(a, 'A3')-2.5709)*20/1.67*pi/180;
		float el_left = 0.0f;  // (rad) left elevon
		float el_right = 0.0f; // (rad) right elevon
	} control_vals;
	...
	struct Airplane_State
	{
		...
		float pd = 0.0f;             // (m)initial Down position(negative altitude)
		float u = 10.0f;              // (m / s) initial velocity along body x - axis
		...
		float q = 0.0f;              // (rad/s) initial body frame pitch rate
		float r = 0.0f;              // (rad/s) initial body frame yaw rate
		float Va = 0.0f;             // (m/s) airspeed
		float alpha = 0.0f;          // (rad) angle of attack
		float beta = 0.0f;           // (rad) side slip angle
		Euler eul;
		Quaternion quat;
	} plane;
